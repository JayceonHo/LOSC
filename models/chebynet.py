...
class ChebNetLayer(nn.Module):
    ...
    def forward(self, x, L):
        T_k = chebyshev_polynomial(L, self.K)  # compute Chebyshev [T_0, T_1, ..., T_K]， each with shape (B, N, N)
        x_tilde = []
        for k in range(self.K+1):
            x_tilde.append(torch.bmm(T_k[k], x))  # T_k(L) * x，(B, N, in_features)
        x_tilde = torch.stack(x_tilde, dim=1)  # (B, K+1, N, in_features)
        x_tilde = torch.einsum('bkni,kio->bno', x_tilde, self.weights)  
        return x_tilde
...
def normalize_laplacian(A, normalize=False):
    ...
    if normalize is True:
        ...
        I = torch.eye(A.shape[1]).unsqueeze(0).repeat(A.shape[0], 1, 1).to(A.device)
